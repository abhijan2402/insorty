...
const InfolwRml = ({ outSideData, index }) => {
  ...
  // console.log(outSideData, "LiqiorDATA");
  ...
  return (
    <>
      <tr key={index}>
        ...
        <td className="tg-0lax">{partyName}</td>

        <td className="tg-0lax">{liquor?.brandName}</td>
        <td className="tg-0lax">{liquor?.quantityInML}</td>
        <td className="tg-0lax">{number}</td>
        <td className="tg-0lax">
          {(Number(rate.$numberDecimal) || 0).toFixed(2)}
        </td>
        <td className="tg-0lax">{(Number(total) || 0).toFixed(2)}</td>

        <td className="tg-0lax">{comment}</td>
      </tr>
    </>
  );
};
