...
const EnglishBearForm = ({ index, liquors, englishBear, total }) => {
  ...
  const quan750 = englishBear.sizes.find(
    (element) => element.quantityInML === 750
  );
  const quan330 = englishBear.sizes.find(
    (element) => element.quantityInML === 330
  );
  const quan180 = englishBear.sizes.find(
        ...
        <td>
          <div className="flex gap-2 items-center">
            <input
              type="number"
              value={
                quan750?.currentStock *
                Number(quan750?.averageRate.$numberDecimal)
              }
              readOnly
              className="commonSmallForm"
            />
            ...
            <input
              type="number"
              value={
                quan180?.currentStock *
                Number(quan180?.averageRate.$numberDecimal)
              }
              readOnly
              className="commonSmallForm"
            />
