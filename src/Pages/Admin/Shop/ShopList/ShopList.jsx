      ...
      await fetch(`${process.env.REACT_APP_API_URL}/admin/createShop`, {
        ...
        .then((data) => {
          if (data.success === true) {
            swal(
              `${
                shopType.charAt(0).toUpperCase() +
                shopType.slice(1).toLowerCase()
            ...
            Swal.fire({
              icon: "error",
              title: `${
                data?.message ? data?.message : "some server error occurred"
              }`,
        ...
        .catch((err) => {
          Swal.fire({
            icon: "error",
            title: `${
              err?.message ? err?.message : "some server error occurred"
            }`,
            showConfirmButton: false,
            timer: 1500,
          });
          ...
          <table className=" removeCommonWSpace">
            <tbody>
              {shops &&
                ...
                shops.data
                  .sort((a, b) =>
                    a?.shopId.name?.localeCompare(b?.shopId?.name)
                  )
                  .filter((shop) => {
                    if (filter === "SHOP") {
                  ...
                  .filter((shop) => shop.shopId.isActive === true)
                  .map((shop) => {
